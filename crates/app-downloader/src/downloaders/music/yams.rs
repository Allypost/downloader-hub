...
use crate::{
    ...
    downloaders::{generic::GenericDownloader, DownloadFileRequest, DownloadUrlInfo},
};
...
impl Handler for YamsProvider {
    ...
    fn download(&self, download_dir: &Path, song_url: &Url) -> anyhow::Result<PathBuf> {
        ...
        let download_url = Self::get_download_url(song_url).map_err(|e| {
        ...
        let song_zip_path = GenericDownloader
            .download_one(
                &DownloadFileRequest::new(&download_url, download_dir),
                &DownloadUrlInfo::from_url(&download_url),
            )
        ...
        let song_file_path =
            Self::extract_song_from_zip(download_dir.to_path_buf(), song_zip_path.path)?;
...
impl YamsProvider {
    #[tracing::instrument]
    fn extract_song_from_zip(download_dir: PathBuf, zip_path: PathBuf) -> anyhow::Result<PathBuf> {
    ...
    fn get_download_url(song_url: &Url) -> anyhow::Result<String> {
        debug!("Getting song download URL");
        let download_id = Self::initialize_song_download(song_url)?;
        Self::wait_for_song_to_finish(download_id)
    }

    fn initialize_song_download(song_url: &Url) -> anyhow::Result<YamsId> {
    ...
    fn wait_for_song_to_finish(download_id: YamsId) -> anyhow::Result<String> {
